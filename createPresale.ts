...
const CONFIG = {
  ...
  creatorSecretKey: Uint8Array.from([214, 155, 243, 54, 0, 39, 204, 63, 111, 22, 24, 16, 59, 132, 229, 139, 102, 168, 205, 190, 172, 220, 77, 8, 23, 60, 206, 66, 255, 101, 118, 50, 214, 39, 229, 246, 185, 150, 1, 253, 61, 34, 196, 153, 73, 123, 238, 254, 123, 95, 190, 80, 251, 86, 42, 143, 131, 107, 249, 102, 103, 6, 86, 84]),

  lxMintAddress: "5r4DrzMSKWUGiqo8KfN5T1hZk5RRiQALRHTE2NUPMjtA",
  ...
  getTokenPriceInSOL(solPrice: number) {
...
const solToBN = (sol: number) =>
  new BN(Math.floor(sol * LAMPORTS_PER_SOL));

const tokenToBN = (amount: number, decimals: number) =>
  new BN(amount).mul(new BN(10).pow(new BN(decimals)));

const minutesToBN = (m: number) => new BN(m * 60);
...
async function main() {
  ...
  const creatorKeypair = Keypair.fromSecretKey(CONFIG.creatorSecretKey);
  ...
  const explorer = CONFIG.rpcUrl.includes("devnet")
